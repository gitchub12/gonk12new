; snap and test fixed.ahk
; IMPORTANT: This is AutoHotkey v1.1 code - DO NOT use v2 syntax!
; v1 uses commands with commas (MsgBox, Hello) not functions MsgBox("Hello")
; DO NOT ADD #Requires DIRECTIVE - it causes version conflicts!
#NoEnv
...
; CLIPBOARD MONITOR toggle button
...
guiX := MonitorWorkAreaRight - 260
guiY := (MonitorWorkAreaBottom - MonitorWorkAreaTop - 160) / 2 + MonitorWorkAreaTop  ; Adjusted for fewer buttons
Gui, Main:Show, w108 h160 x%guiX% y%guiY%  ; Reduced height
...
snapCount := 0
currentPort := 8000
excludePatterns := ".md,.bin,.exe,.dll,.tmp,node_modules,.git,.bat"
...
MonitorClipboard:
    ...
    ; SIMPLE FILE DETECTION - just look for any word ending in .js, .html, or .json
    ; Must have space or / or \ before, and space after
    if (RegExMatch(firstThreeLines, "(.*?)([\w\-]+\.(js|html|json))(\s|$)", match)) {
        targetFile := match2
        fullLineWithFile := match1 . match2 . match3
        
        ; Find which line contains the file
        Loop, 3
        {
            if (A_Index <= clipLines0 && InStr(clipLines%A_Index%, targetFile)) {
                fullLineWithFile := clipLines%A_Index%
                break
            }
        }
        
        ToolTip, Found file: %targetFile%, %tooltipX%, %tooltipY%
        Sleep, 500
    }
    ...
    if (targetFile != "") {
        ...
        ; If not found, search in data folder and subfolders
        if (foundPath = "" && FileExist(A_ScriptDir . "\data")) {
            searchLog .= "- Data folders"
            Loop, Files, %A_ScriptDir%\data\*.*, R
            {
        ...
        if (foundPath != "") {
            ...
            ToolTip, FILE NOT FOUND: %targetFile%`nLine: %fullLineWithFile%`n`nClick YES to create file`, NO to cancel, %tooltipX%, %tooltipY%
            
            ; Show a message box asking if they want to create the file
            Gui, Main:+OwnDialogs
            MsgBox, 4, File Not Found, %targetFile% not found.`n`nFull line: %fullLineWithFile%`n`nCreate it in the main folder?
            if (A_MsgBoxResult = "Yes") {
                ; Create the file with clipboard contents
                newFilePath := A_ScriptDir . "\" . targetFile
                FileAppend, %currentClip%, %newFilePath%
                
                if (ErrorLevel) {
                    ToolTip, ERROR: Failed to create %targetFile%, %tooltipX%, %tooltipY%
                    SetTimer, RemoveTooltip, 3000
                } else {
                    ; File created successfully
                    ToolTip, Created: %targetFile% in main folder, %tooltipX%, %tooltipY%
                    
                    ; Open or activate Explorer window
                    explorerPath := "explorer.exe /select,""" . newFilePath . """"
                    Run, %explorerPath%
                ...
                ToolTip, Cancelled file creation, %tooltipX%, %tooltipY%
                SetTimer, RemoveTooltip, 2000
            }
        }
    } else {
...
MidSnap:
    ...
    ; --- Main folder files (no change here) ---
    mainFiles := ""
    mainCount := 0
    Loop, Files, %A_ScriptDir%\*.*
    {
    ...
    ; --- MODIFICATION START ---
    ; --- Dynamically process all other subfolders ---
    Loop, Files, %A_ScriptDir%\*, D
    {
        folderName := A_LoopFileName
        ; Skip excluded folders like 'test' or any you might add to a list
        if (folderName = "test" or folderName = "archive")
            continue
        ...
        ; Loop through files recursively in the current folder
        Loop, Files, %A_ScriptDir%\%folderName%\*.*, R
        {
            ; Redundant check for test/archive sub-sub-folders just in case
            if (InStr(A_LoopFileFullPath, "\test\") or InStr(A_LoopFileFullPath, "\archive\"))
                continue
            ...
            StringReplace, relativePath, relativePath, \, /, All
            folderContent .= relativePath . " [" . sizeDisplay . "] " . timeStr . "`n"
        }
        
        ; Add this folder's content to the main snapshot if it has files
        if (folderFileCount > 0) {
            snapshot .= folderNameUpper . " FOLDER (" . folderFileCount . " files):`n" . folderContent . "`n"
        }
    }
    ; --- MODIFICATION END ---
